...
def train(env_name="gym", map_name="CartPole-v1", alg_name="reinforce_baseline"):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if done:
                ...
                elif alg_name == "reinforce_baseline":
                    agent.learn(rewards, log_probs, np.array(states))
...
def evaluate(env_name="gym", map_name="CartPole-v1", alg_name="reinforce_baseline"):
    env = e_REGISTRY[env_name](map_name)
    agent = a_REGISTRY[alg_name](env)
